...
if(isset($_SESSION['user']) && $_SESSION['user'][0]['type_user']!='admin')
{
					...
					<div class="widget-content nopadding">
						<table class="table table-bordered table-striped">
							...
							<tbody>
								...
								$x=count($getallproduct);

								$sosanpham1trang=4;
								
								$tongsotrang=ceil($x/$sosanpham1trang);
								
								$productpage=$product->PhanTrang($_GET['p'],$sosanpham1trang);
								
								foreach($productpage as $value){
								
								?>
							<tr class="">
								<td><img src="./public/images/<?php echo $value['image'] ?>" /></td>
								<td><?php echo $value['name'] ?></td>
								<?php// var_dump($nameprotype); ?>
								...
								<td><?php echo substr($value['description'], 0,500)?></td>
								<td><?php echo $value['price'] ?></td>
								<td>
									<a href="form_editproduct.php?p=<?php echo $value['id'] ?>" class="btn btn-success btn-mini">Edit</a>
									<a href="delete.php?p=<?php echo $value['id']?>" class="btn btn-danger btn-mini">Delete</a>
								</td>
							</tr>
								<?php } ?>
						</tbody>	
