...
$showuser=$user->getUserbyID($_GET['user']);
$showalluser=$user->getTypeuser();
?>
<!DOCTYPE html>
<html lang="en">
<head>
				...
				<div class="widget-box">
					<div class="widget-content nopadding">
						<table class="table table-bordered table-striped">
							...
							<tbody>
							<tr class="">
                                <form action="update.php?user=<?php echo $showuser[0]['ID'] ?>" method="post">
								<td> <input type="text" name="username" value="<?php echo $showuser[0]['username']?>"></td>
								<td> <input type="password" name="password" value="<?php echo $showuser[0]['password']?>"></td>
                                <td> 
                                <div class="controls">
								<select name="type_id">
									<?php	foreach($showalluser as $value) {
										?>
										<option <?php if($showuser[0]['type_user']==$value['id']) { echo "selected=\"selected\""; } ?> value="<?php echo $value['id'] ?>"><?php echo $value['type_user'] ?></option>
									<?php } ?>
								</select> *
                                </div>
                                </td>
